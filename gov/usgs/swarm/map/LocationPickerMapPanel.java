...
public class LocationPickerMapPanel extends JPanel{
	...
	private Integer pressedX;
	private Integer pressedY;
...
//					parent.setStatusText(" ");
	...
	public class MapImagePanel extends JPanel
	{
		...
		private GeneralPath crossPath = new GeneralPath();
			...
			if (renderer == null || mapImage == null) {
				...
				if (dragMode == DragMode.DRAG_MAP && mouseDown != null && mouseNow != null) {
					...
					if (attempt != null) {
						...
						g2.draw(getCrossByLocation(attempt.getLongitude() - dx, attempt.getLatitude() - dy));
					}
					
				} else if (dragDX != Integer.MAX_VALUE && dragDY != Integer.MAX_VALUE) {
					// System.out.println("when this works dragDX=" + dragDX +
					// " dragDY=" + dragDY);
					...
					if (attempt != null) {
						g2.setColor(Color.RED);
						g2.setStroke(stroke2);
						g2.draw(getCrossByLocation(attempt.getLongitude(), attempt.getLatitude()));
					}
					if(pressedX!= null){
						g2.setPaint(Color.RED);
						g2.fillRect(pressedX-6, pressedY-6,6, 6);
						getPickedLongAndLatitude();
					}
				}
				...
				if (dragRectangle != null) {
					if (dragMode == DragMode.BOX && mouseNow != null) {
						...
						g2.drawLine(mouseDown.x - LocationPickerMapPanel.this.getInsets().left,
								mouseDown.y - LocationPickerMapPanel.this.getInsets().top, mouseNow.x
										- LocationPickerMapPanel.this.getInsets().left, mouseNow.y - LocationPickerMapPanel.this.getInsets().top);
		...
		private GeneralPath getCrossByLocation(double locationX, double locationY)
		{
			crossPath.reset();
			crossPath.moveTo(locationX - 5, locationY - 5);
			crossPath.lineTo(locationX + 5, locationY + 5);
			crossPath.moveTo(locationX - 5, locationY + 5);
			crossPath.lineTo(locationX + 5, locationY - 5);
			crossPath.closePath();
			return crossPath;
		...
		public void showSelectedAttempt(Attempt attempt)
		{
	...
	public Point2D.Double  getPickedLongAndLatitude(){
		System.out.println(pressedX + " ---- " +  pressedY);
		 Point2D.Double loc = getLonLat(pressedX, pressedY);
