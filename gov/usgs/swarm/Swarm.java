...
/**
 * The main UI and application class for Swarm. Only functions directly pertaining to the UI and overall application
 * operation belong here.
 *
...
public class Swarm extends JFrame {
    ...
    public void createUI() {
        ...
        */
        ...
        if (config.windowMaximized) this.setExtendedState(Frame.MAXIMIZED_BOTH);
    ...
    public void fireTimeChanged(double j2k) {
        ...
        for (int i = ls.length - 2; i >= 0; i -= 2)
            if (ls[i] == TimeListener.class) ((TimeListener)ls[i + 1]).timeChanged(j2k);
    }
    ...
    public void setChooserVisible(boolean vis) {
        if (vis) {
            ...
            if (isChooserVisible()) config.chooserDividerLocation = split.getDividerLocation();
    ...
    public void closeApp() {
        ...
        if (this.getExtendedState() == Frame.MAXIMIZED_BOTH) config.windowMaximized = true;
        else {
        ...
        if (waveClipboard.isMaximum()) config.clipboardMaximized = true;
        else {
        ...
        if (mapFrame.isMaximum()) config.mapMaximized = true;
        else {
    ...
    public void saveLayout(String name) {
        ...
        while (!done) {
            if (name == null) {
                name =
                        (String)JOptionPane.showInputDialog(Swarm.getApplication(), "Enter a name for this layout:",
            ...
            if (name != null) {
                if (Swarm.config.layouts.containsKey(name)) {
                    ...
                    if (!fixedName) {
                        int opt =
                                JOptionPane.showConfirmDialog(Swarm.getApplication(),
    ...
    public SwarmLayout getCurrentLayout() {
        ...
        if (mapFrame.isVisible()) mapFrame.saveLayout(cf, "map");
    ...
    public void addInternalFrame(final JInternalFrame f, boolean setLoc) {
        ...
        if (setLoc) f.setLocation(frameCount * 24, frameCount * 24);
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
    ...
    public void flush(int position) {
        ...
        if (waveClipboard.isSelected()) {
            bingo = waveClipboard;
        } else for (JInternalFrame frame : frames)
            if (frame.isSelected()) bingo = frame;

        if (bingo != null) switch (position) {
        case LEFT:
    ...
    public void tileKioskFrames() {
        ...
        for (JInternalFrame frame : frames) {
            if (frame.isVisible() && frame instanceof Kioskable) ks.add(frame);
        }
        ...
        for (JInternalFrame frame : ks) {
            if (frame instanceof MapFrame) mapCount++;
            else if (frame instanceof HelicorderViewerFrame) heliCount++;
            else if (frame instanceof MultiMonitor) monitorCount++;
        }
    ...
    public void tileHelicorders() {
        ...
        for (JInternalFrame frame : frames) {
            if (frame instanceof HelicorderViewerFrame) hcs.add((HelicorderViewerFrame)frame);
        }
    ...
    public void tileWaves() {
        ...
        int wc = 0;
        for (JInternalFrame frame : frames) {
            if (frame instanceof WaveViewerFrame) wc++;
        }
    ...
    private void parseKiosk() {
        ...
        if (config.kiosk.startsWith("layout:")) {
            ...
            if (sl != null) {
                lastLayout = layout;
                sl.process();
            } else Swarm.logger.warning("could not start with layout: " + layout);
        }
        boolean set = false;
        ...
        if (config.kiosk.equals("true")) set = true;

        if (set) toggleFullScreenMode();
        else Swarm.logger.warning("no helicorders, skipping kiosk mode.");
    }
    ...
    public void optionsChanged() {
        for (JInternalFrame frame : frames) {
            if (frame instanceof HelicorderViewerFrame) {
                ...
                if (!config.durationEnabled) hvf.getHelicorderViewPanel().clearMarks();
    ...
    public static void main(final String[] args) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                if (Swarm.config.isKiosk()) swarm.parseKiosk();
            }
        });
    }
