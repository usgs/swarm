...
public class WebServicesClient extends AbstractDataRecordClient {
     ...
     * 
     * @param net
     *            the network filter or empty if none.
     * @param sta
     *            the station filter or empty if none.
     * @param loc
     *            the location filter or empty if none.
     * @param chan
     *            the channel filter or empty if none.
     */
     ...
     * @param chan
     *            the channel filter or empty if none.
     * @param wsDataSelectUrl
     *            the web services data select URL text.
     * @param wsStationUrl
     *            the web services station URL text.
            ...
            stationClient = new WebServiceStationXmlClient(wsStationUrl, net, sta, loc, chan, date) {
                public void processChannel(ChannelInfo ch) {
                    ...
                    if (last_sta.compareTo(ch.getStation()) != 0) {
                        ...
                        if (num_stations > 0)
                            getSource().fireChannelsProgress(progress_id, (double) station_cnt / (double) num_stations);
                        station_cnt++;
                    }
                }
            };
    ...
    public List<String> getChannels() {
        ...
        if (channelList.size() != 0) {
            ...
            if (stationClient.isAllNetworks()) {
                ...
                if (error == null) {
                    /*
                     * int cnt = 0; final List<StationInfo> stationList =
                     * stationClient .getStationList(); final int ns =
                     * stationList.size(); for (StationInfo station :
                     * stationList) { getSource().fireChannelsProgress(id,
                     * (double) cnt / (double) ns); cnt++;
                     * stationClient.setCurrentStation(station); error =
                     * stationClient.fetchChannels(); if (error != null) {
                     * break; } }
                     */
                    stationClient.setCurrentStation(null);
                    error = stationClient.fetchChannels();
        ...
        final DataSelectReader reader = new DataSelectReader(wsDataSelectUrl) {
            ...
            public boolean processRecord(DataRecord dr) {
                try {
                    ...
                    WebServiceUtils.warning("could not get web service raw data (" + channelInfo + "): "
                            + ex.getMessage());
                }
                return true;
            }
