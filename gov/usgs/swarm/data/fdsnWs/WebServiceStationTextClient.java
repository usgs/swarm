...
public class WebServiceStationTextClient extends
	...
	public static WebServiceStationTextClient createClient(String[] args) {
		...
		return new WebServiceStationTextClient(baseUrlText, net, sta, loc,
				chan, date);
	}
		...
		case STATION:
			...
			if (columns.length < minNumColumns) {
				...
				double latitude = StationInfo.parseDouble(getColumnText(
						columns, 2));
				double longitude = StationInfo.parseDouble(getColumnText(
						columns, 3));
				String siteName = getColumnText(columns, 5);
				processStation(createStationInfo(station, network, latitude,
						longitude, siteName));
			}
			break;
		case CHANNEL:
			...
			if (columns.length < minNumColumns) {
				...
				double longitude = StationInfo.parseDouble(getColumnText(
						columns, 5));
				processChannel(createChannelInfo(station, channel, network,
