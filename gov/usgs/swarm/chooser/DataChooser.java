...
public class DataChooser extends JPanel {
	...
	private static final String OPENING_MESSAGE = Messages
			.getString("DataChooser.treeOpening"); //$NON-NLS-1$
	...
	public DataChooser() {
		...
		createTree();
		createNearest();
		split = SwarmUtil.createStrippedSplitPane(JSplitPane.VERTICAL_SPLIT,
				treeScrollPane, nearestPanel);
				...
				new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						SwingUtilities.invokeLater(new Runnable() {
							public void run() {
								EditDataSourceDialog d = new EditDataSourceDialog(
										null);
								d.setVisible(true);
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (servers != null && servers.size() > 0) {
							...
							if (sds.isStoreInUserConfig()) {
								String selected = servers.get(0).getSource()
										.toConfigString();
								EditDataSourceDialog d = new EditDataSourceDialog(
										selected);
								d.setVisible(true);
	...
	private double getTime() {
		...
		String t0 = ((JTextField) timeBox.getEditor().getEditorComponent())
				.getText();
	...
	private void createActionBar() {
						...
						if (channels != null) {
							...
							for (Pair<ServerNode, String> pair : channels) {
								Swarm.getApplication()
										.openHelicorder(pair.item1.getSource(),
												pair.item2, j2k);
							}
						}
						return null;
			...
			public void actionPerformed(ActionEvent e) {
				SwingWorker worker = new SwingWorker() {
					public Object construct() {
						...
						if (channels != null) {
							for (Pair<ServerNode, String> pair : channels) {
								Swarm.getApplication().openRealtimeWave(
										pair.item1.getSource(), pair.item2);
							}
						}
						return null;
					}
				...
				SwingWorker worker = new SwingWorker() {
					public Object construct() {
						...
						Collections.sort(channels,
								new Comparator<Pair<ServerNode, String>>() {
									public int compare(
											Pair<ServerNode, String> o1,
											Pair<ServerNode, String> o2) {
										return o1.item2.compareTo(o2.item2);
									}
								});
						if (channels != null) {
							for (Pair<ServerNode, String> pair : channels) {
								Swarm.getApplication().loadClipboardWave(
										pair.item1.getSource(), pair.item2);
							}
						}
			...
			public void actionPerformed(ActionEvent e) {
				SwingWorker worker = new SwingWorker() {
					public Object construct() {
						...
						if (channels != null) {
							for (Pair<ServerNode, String> pair : channels) {
								Swarm.getApplication().monitorChannelSelected(
										pair.item1.getSource(), pair.item2);
							}
						}
						return null;
		...
		timeBox.getEditor().getEditorComponent()
	...
	private class DataSourceOpener extends SwingWorker {
		...
		public void finished() {
			...
			if (finishListener != null)
				finishListener.actionPerformed(new ActionEvent(
						DataChooser.this, id, source.getSource().getName()));
		}
	}
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				for (i = 0; i < rootNode.getChildCount(); i++) {
					String s = ((ServerNode) rootNode.getChildAt(i))
							.getSource().getName();
	...
	public void addServers(final Map<String, SeismicDataSource> servers) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				List<String> list = Collections.list(Collections
						.enumeration(servers.keySet()));
	...
	public void setNearest(final String channel) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				DefaultListModel model = (DefaultListModel) nearestList
						.getModel();
				model.removeAllElements();
				for (Pair<Double, String> item : nrst)
					model.addElement(String.format("%s (%.1f km)", item.item2,
							item.item1 / 1000));
			}
		});
	}
	 ...
	 * @param channels
	 */
	private void populateServer(final ServerNode node,
			final List<String> channels, final boolean expandAll,
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				GroupNode allNode = new GroupNode(Messages
						.getString("DataChooser.allGroup")); //$NON-NLS-1$
								...
								if (nn == null) {
									...
									for (j = 0; j < cn.getChildCount(); j++) {
										if (cn.getChildAt(j) instanceof GroupNode) {
											GroupNode ogn = (GroupNode) cn
													.getChildAt(j);
											if (nn.name
													.compareToIgnoreCase(ogn.name) <= 0)
												break;
										}
									}
	...
	private List<Pair<ServerNode, String>> getSelectedLeaves(TreePath[] paths) {
		...
		for (int i = 0; i < paths.length; i++) {
			...
			if (node.isLeaf() && node instanceof ChannelNode) {
				selections.add(new Pair<ServerNode, String>(serverNode,
						((ChannelNode) node).channel));
			} else if (!node.isLeaf()) {
				Set<String> channels = getGroupChannels((GroupNode) node);
				for (String ch : channels)
					selections
							.add(new Pair<ServerNode, String>(serverNode, ch));
			}
	...
	private class CellRenderer extends DefaultTreeCellRenderer {
		...
		public Component getTreeCellRendererComponent(JTree tree, Object value,
			...
			if (value instanceof ProgressNode) {
				...
				super.getTreeCellRendererComponent(tree, node.getLabel(), sel,
						exp, leaf, row, focus);
				setIcon(icon);
				return this;
			} else {
