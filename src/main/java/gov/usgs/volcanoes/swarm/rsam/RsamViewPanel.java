...
public class RsamViewPanel extends JComponent implements SettingsListener {
   ...
   * { String ft = ""; switch (settings.filter.getType()) { case BANDPASS: ft = "Band pass [" +
   * settings.filter.getCorner1() + "-" + settings.filter.getCorner2() + " Hz]"; break; case
   * HIGHPASS: ft = "High pass [" + settings.filter.getCorner1() + " Hz]"; break; case LOWPASS: ft =
   * "Low pass [" + settings.filter.getCorner1() + " Hz]"; break; default: break; } TextRenderer tr
   * = new TextRenderer(x, y, ft); tr.horizJustification = horizJustification; tr.vertJustification
   * = vertJustification; tr.color = Color.red; return tr; }
   */
}
