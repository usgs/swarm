...
public class AboutDialog extends JDialog implements Runnable {
	...
	private final static int ATTRIBUTION_LOC_Y = TITLE_LOC_Y + TITLE_HEIGHT + PANEL_PADDING;
	private final static int ATTRIBUTION_HEIGHT = 175;

	private final static int MEMORY_LOC_Y = ATTRIBUTION_LOC_Y + ATTRIBUTION_HEIGHT + PANEL_PADDING;
	private final static int MEMORY_HEIGHT = 100;

	private final static int BUTTON_PAD_HEIGHT = 11;
	private final static int BUTTON_LOC_Y = MEMORY_LOC_Y + MEMORY_HEIGHT + BUTTON_PAD_HEIGHT;
	private final static int BUTTON_HEIGHT = 30;
	private final static int BG_WIDTH = 357;
	...
	private JTextPane getAttributionPane() {
		...
		pane.setBorder(LineBorder.createBlackLineBorder());
		pane.setContentType("text/html");
		pane.setText("<HTML>" + "<DIV style=\"text-align: center;\"><B>Developed by:</B><br>U.S. Geological Survey"
				+ "<p><B>With contributions from:</B>" + "<BR>Instrumental Software Technologies, Inc. (ISTI)"
				+ "<BR>The Incorporated Research Institutions for Seismology (IRIS)"
				+ "<P><B>Primary developers:</b><BR>Dan Cervelli<BR>Tom Parker" + "</DIV></HTML>");
		return pane;
	}
